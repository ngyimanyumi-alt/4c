...
test('duty section keeps a wider horizontal layout while remaining responsive', () => {
  ...
  assert.match(html, /查看唯一放學後值日組別（兩位值日生與分工）。/);
  assert.doesNotMatch(html, /早會/);
...
test('demo data provider preserves student numbers and persists absence and duty offset updates', async () => {
  ...
  try {
    ...
    await classDataApi.upsertDutyOverride('2026-09-05', '放學後', 2);
    let offsets = await classDataApi.listDutyOverrides();
    const createdOffset = offsets.find((item) => item.date === '2026-09-05' && item.slot === '放學後');
    ...
    assert.equal(offsets.some((item) => item.date === '2026-09-05' && item.slot === '放學後'), false);
  } finally {
    delete global.window;
  }
});
